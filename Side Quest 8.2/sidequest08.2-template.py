...
def contains_unique_day(month, possible_birthdays):
    dates = tuple(filter(lambda x: x[0] == month,possible_birthdays))
    for date in dates:
...
def statement1(birthday, possible_birthdays):
    if unique_month(birthday[0],possible_birthdays):
        return False
    else:
        if contains_unique_day(birthday[0],possible_birthdays):
            return False
        else: return True
...
def get_birthday(possible_birthdays):
    new_list = tuple(filter(lambda x: statement1(x,possible_birthdays), possible_birthdays))
    new_new_list = tuple(filter(lambda x: statement2(x,new_list), new_list))
    birthday = tuple(filter(lambda x: statement3(x,new_new_list), new_new_list))
    return birthday
